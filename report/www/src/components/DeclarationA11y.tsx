    ...
    declarationUpToDate: {
      true: (
        <Alert
          severity="success"
          title=""
          description={`La déclaration est à jour (moins de 3 ans) : ${data.declarationDate.toLocaleDateString("fr-FR", { year: 'numeric', month: 'long', day: 'numeric' })}`}
        ></Alert>
      ),
      false: (
        <Alert
          severity="warning"
          title=""
          description={`La déclaration n'est pas à jour (plus de 3 ans) ou sa date n'a pas été détectée : ${data.declarationDate.toLocaleDateString("fr-FR", { year: 'numeric', month: 'long', day: 'numeric' }) ?? "non trouvée"}`}
        ></Alert>
      ),
    },
  };
